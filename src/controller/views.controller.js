...
class ViewsController {
  ...
  async renderFilteredProducts(req, res) {
    try {
      const { page, limit, sort, query } = req.query;
      const titleFilter = req.query.title || '';
      const encodedTitleFilter = encodeURIComponent(titleFilter);
      const user = new userDTO(req.session.user);
      const totalPages = parseInt(page);
      let allFilteredProducts = [];
      for (let currentPage = 1; currentPage <= totalPages; currentPage++) {
        const result = await serviceProducts.getFilteredProductsService(currentPage, limit, sort, query, encodedTitleFilter);
        const productsOnPage = result.docs;
        const filteredProductsOnPage = productsOnPage.filter((product) => {
          return product.title.toLowerCase().includes(titleFilter.toLowerCase());
        });
        allFilteredProducts = allFilteredProducts.concat(filteredProductsOnPage);
      }
      ...
      const nextPage = parseInt(page) + 1;
      const prevPage = parseInt(page) - 1;
      const nextPageUrl = `/products/?page=${nextPage}&limit=${limit}&sort=${sort}&title=${titleFilter}`;
      const prevPageUrl = `/products/?page=${prevPage}&limit=${limit}&sort=${sort}&title=${titleFilter}`;
      const productsContext = {
        isAdmin: user.isAdmin,
        isPremium: user.isPremium,
        userCartID: user.cartID,
        session: req.session.user,
        productsVisualice: paginatedProducts,
        paginationInfo: {
          totalDocs: allFilteredProducts.length,
        },
        nextPageUrl: nextPageUrl,
  ...
  async renderRealTimeProductsAdmin(req, res) {
    try {
      const user = req.session.user;
      if (!user) {
        return res.status(401).json({ message: 'Unauthorized' });
      ...
      const nextPage = parseInt(page) + 1;
      const nextPageUrl = `/realtimeproductsadmin?page=${nextPage}&limit=${limit}&sort=${sort}`;
      const realTimeProductsUser = new userDTO(req.session.user);
      const realTimeProductsContext = {
        isAdmin: realTimeProductsUser.isAdmin,
        isPremium: realTimeProductsUser.isPremium,
      ...
      res.render('realtimeproductsadmin', { realTimeProductsRender });
      logger.info('realTimeProducts function called successfully.');
    } catch (error) {
      logger.error(error);
    }
